<div>
    ...
    <div class="form-row">
        ...
        <div class="form-group">
            <button class="btn btn-outline-primary" wire:click="crearCliente" data-toggle="modal"
    ...
    <div class="table-responsive">
        <table class="table table-sm table-bordered">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @if ($clientes->count())
                    @foreach ($clientes as $cliente)
                        <tr>
                            <td>{{ $cliente->id }}</td>
                            <td>
                                @if ($cliente->foto) <img src="{{ $cliente->foto }}"
                            ...
                            <td class="">
                                <button type="button" class="btn btn-sm btn-success mb-1" data-toggle="modal"
                                    data-target="#clienteModal" wire:click="actualizarCliente({{ $cliente }})">
                                    {{ __('Actualizar') }}
                                </button>
                                <button class="btn btn-sm btn-danger" wire:click="delete('{{ $cliente->id }}')"
                ...
                @else
                    <tr>
                        <td class="px-6 py-4 text-sm whitespace-no-wrap" colspan="16">No Results Found</td>
                    </tr>
                @endif
            </tbody>
        </table>
