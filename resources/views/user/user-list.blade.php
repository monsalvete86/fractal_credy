<div class="table-responsive">
    ...
    <div class="form-group row">
        <div class="col-12">
            <button class="btn btn-outline-primary" wire:click="newUser()" data-toggle="modal"
                    ...
                    <td>
                        @if ($user->estado == 1)
                            <button type="button" class="btn btn-success" data-toggle="modal"
                                data-target="#exampleModal" wire:click="showModal({{ $user }})">
                                <span class="material-icons">create</span>

                            </button>
                            <button class="btn btn-danger"
                                onclick="inactivar('{{ $user->email }}','{{ $user->name }}')">
                                <span class="material-icons">cancel</span>
                            </button>
                        @else
                            <button class="btn btn-warning"
                                onclick="activar('{{ $user->email }}','{{ $user->name }}')">
